...
var validator = (function($){
	...
	*/
	function checkField() {
        ...
        field.data( 'val', field[0].value);	// cache the value of the field and trim it
		data = field.data();
	...
	*/
	...
	function checkAllWithHide($form) {
	    ...
	    var that = this,
			submit = true, // save the scope
            fieldsToCheck = $form.find(':input').setDefaults({ ignore: ":hidden:not(.hidden-validation)" });
	...
	function checkSelect(drpId) {
	    var va = $(drpId).val();
	    if ($(drpId).val() != '' && $(drpId).val() != '0') {
	        console.warn('element not found');
	        return false;
	    }
	    var that = this,
	    ...
	    submit = submit * checkField.apply($(drpId));
	    return !!submit;  // casting the variable to make sure it's a boolean
	}
	return {
		defaults 	: defaults,
